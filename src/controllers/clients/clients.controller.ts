import { Body, Controller, Delete, Get, Inject, Logger, Param, Post, Put, Render, Req, Res, UseFilters, UseGuards } from '@nestjs/common';
...
export class ClientsController extends BaseController {
  ...
  @Post()
  async storeClient(@Body() createClientDto: IClientDto, @Req() req: Request, @Res() res: Response) {
    const errors: string[] = [];

    if (!createClientDto.first_name || createClientDto.first_name.length <= 0) {
      errors.push('Client first name required')
      return res.status(400).json({ message: 'Unable to create client.', errors: errors })
    }
  ...
  @Put(':id')
  async updateClient(@Body() updateClientDto: IClientDto, @Param('id') id: number, @Req() req: Request, @Res() res: Response) {
  ...
  @Delete(':id')
  async deleteClient(@Param('id') id: number, @Req() req: Request, @Res() res: Response) {
