...
export class OrdersService {
    ...
    constructor(dataSource: DataSource,
        ...
        this.productServicePriceRepository = dataSource.getRepository(ProductServicePrice);
    }
    ...
    async getOrdersAvailableForUser(user: IPrincipal | number | string, startAt: number = 0, size: number = 50) {
        ...
        if (!hasElevatedPrivileges)
            query = { ...query, where: { ...(query.where || {}), recorderId: dbUser.id } };

        const totalRecords = await this.ordersRepository.count({ where: query.where });
        const totalPages = Math.ceil(totalRecords / size) + 1;
        const isFirstPage = startAt == 0;
        const isLastPage = (await this.ordersRepository.count({ where: query.where, skip: (startAt + 1) * size })) <= 0;
