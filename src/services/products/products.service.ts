...
export class ProductsService {
  ...
  constructor(datasource: DataSource) {
    ...
    this.productServiceRepository = datasource.getRepository(ProductServicePrice);
  }
  ...
  async createProduct(dto: IProductDto) {
    ...
    if (exists) {
      this.logger.warn(`Failed product creation attempt - Product with same name already exists.`)
    ...
    dto.services.forEach(async service => {
      ...
      if (service.type == 'normal') 
        ...
        productServicePrice.fastPrice = service.price
  ...
  async deleteProduct(id: number) {
    ...
    if (product) {
      const product_prices = await this.productServiceRepository.findBy({ productId: id })
      product_prices.forEach(async product_price => await this.productServiceRepository.delete({ id: product_price.id }))
